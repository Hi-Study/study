...
--    수집(정각) 직후 아직 분석 안 된 글을 한 번에 2건씩 처리한다.
--
-- ⚠️ 건수는 **LLM 무료 티어 한도에 맞춘 값**이다. 실측(2026-09-02):
--    Groq 무료 티어는 분당 8,000토큰(TPM) · 하루 200,000토큰(TPD).
--    enrich 한 건이 약 3,000토큰이라 하루에 대략 65건이 상한이다.
--    5건/시간(=120건/일)으로 두면 오후에 일일 한도를 태우고 그날 남은 요청이
--    전부 429 로 실패한다(실제로 그렇게 겪었다). 2건/시간 = 48건/일이 안전선.
--    더 빨리 채우려면 Groq 유료 티어로 올리고 이 숫자를 키운다.
