...
type Init = { title: string; category: Category; tags: string[]; problem: string; solution: string; impact: string };
...
export default function EditForm({ postId, init }: { postId: string; init: Init }) {
  ...
  const submit = () => {
    ...
    setErr("");
    start(async () => {
      const res = await updatePost(postId, { title, category, tags, problem, solution, impact });
