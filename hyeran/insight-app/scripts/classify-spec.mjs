...
export const FLAGS = ["기대와 다른 결과", "직접 만들기", "개발 과정에 AI", "스택 심화"];
...
【flags】 해당하는 것을 모두. 없으면 빈 배열.
  ...
  스택 심화        판정 질문은 하나다:
                   ...
                   붙이는 경우:
                   - 제목이나 본론에 특정 기술 이름이 나오고
                     (프레임워크·언어·라이브러리·미들웨어·DB·프로토콜)
                     그 기술을 쓰는 사람만 이해할 수 있는 내용인 경우
                   - 코드 설계·아키텍처 패턴·동작 원리·성능 튜닝·
                     마이그레이션 방법이 본론인 경우
                   예: App Router / Airflow 멀티프로세싱 / Kafka Streams /
                       디자인 패턴 적용 / KMP 마이그레이션 / 헥사고날 아키텍처

                   붙이지 않는 경우:
                   - 개발 과정에 AI 를 쓴 이야기 (스택을 몰라도 읽을 값이 있다)
                   - 기술 이름이 나와도 결정·조직·일하는 방식이 본론인 경우
                   - 사용자에게 무엇이 달라졌는지가 본론인 경우

                   판단이 애매하면 붙여라. 읽을 글은 이미 충분히 많다.
